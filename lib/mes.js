...
export async function fetchMesCompleto(mesId) {
  const [gf, gv, oi, cd] = await Promise.all([
  ...
  let tarjetaMotivos = [];
  const idsConTarjeta = gastosVariables.filter((g) => g.categoria === "Tarjeta de crédito").map((g) => g.id);
...
export function varItemTotal(item, tarjetaMotivos) {
  if (item.categoria === "Tarjeta de crédito") {
...
export function calcularTotales(mes, datos) {
  const { gastosFijos, gastosVariables, tarjetaMotivos, otrosIngresos, comprasDolares } = datos;
  const totalOtrosIngresos = otrosIngresos.reduce((s, o) => s + Number(o.monto || 0), 0);
  ...
  const totalGastosVariables = gastosVariables.reduce((s, g) => s + varItemTotal(g, tarjetaMotivos), 0);
...
export async function crearMesSiguiente(userId, mesActual, datos) {
  ...
  const tarjetas = datos.gastosVariables.filter((g) => g.categoria === "Tarjeta de crédito");

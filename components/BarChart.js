...
const SERIES = [
  { key: "ingresos", label: "Ingresos", color: "#7CB88D" },
  { key: "gastos", label: "Gastos", color: "#C97B6B" },
  { key: "remanente", label: "Remanente", color: "#6BA0C9" },
];

// filas: [{ periodoLabel, ingresos, gastos, remanente }]
export default function BarChart({ filas }) {
  if (!filas || filas.length === 0) {
    return <p style={{ color: "#93A99B", fontSize: 13 }}>Todavía no hay meses para graficar.</p>;
  }
  const max = Math.max(
    1,
      ...
      <div style={{ display: "flex", gap: 14, marginBottom: 12, fontSize: 12 }}>
        {SERIES.map((s) => (
          <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 5 }}>
            ...
            <span style={{ color: "#93A99B" }}>{s.label}</span>
          </div>
        ))}
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {filas.map((f) => (
          <div key={f.periodoLabel}>
            <div style={{ fontSize: 13, color: "#EFE9DA", marginBottom: 4 }}>{f.periodoLabel}</div>
            {SERIES.map((s) => {
              ...
              return (
                <div key={s.key} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 3 }}>
                  <div style={{ flex: 1, background: "#182B22", borderRadius: 3, overflow: "hidden", height: 14 }}>
                    <div style={{ width: `${widthPct}%`, background: s.color, height: "100%" }} />
                  </div>
                  <div style={{ width: 90, fontSize: 11, color: "#93A99B", textAlign: "right" }}>{fmtPesos(v)}</div>
                </div>
              );
            })}
          </div>
